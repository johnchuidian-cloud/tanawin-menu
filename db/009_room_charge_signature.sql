-- Touch signature for "charge to room" orders: guests sign at checkout,
-- the PNG lands in a private bucket (staff-read only, like gcash-proofs),
-- and the path is stored on the order as authorization evidence.
...
create policy "anon upload signature" on storage.objects
  for insert to anon with check (bucket_id = 'signatures');
create policy "staff read signatures" on storage.objects
  for select to authenticated using (bucket_id = 'signatures');

-- place_order grows an optional p_signature_url. The default keeps older
-- cached clients (which don't send it) working — PostgREST calls by named
-- argument, so the missing param just takes the default.
drop function public.place_order(text, boolean, text, text, text, jsonb);
...
begin
  ...
  insert into orders (room_number, is_dining_in, payment_intent, gcash_proof_url, note, total, signature_url)
  values (
